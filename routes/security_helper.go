...
func GenerateAccessTokenHandler(w http.ResponseWriter, r *http.Request) {
	...
	if apiKey == "" || secret == "" || !helper.ValidateSecret(apiKey, secret) {
		respondWithError(w, http.StatusUnauthorized, "Invalid apiKey or secret")
		return
	}

	token, err := helper.GenerateAccessToken(apiKey, secret, audience, 3600) // 1 hour
	if err != nil {
